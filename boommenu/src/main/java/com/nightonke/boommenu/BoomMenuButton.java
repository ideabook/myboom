...
public class BoomMenuButton extends FrameLayout
    ...
    // Array reference cannot never be null as initialized
    private HamButton[] hamButtons = new HamButton[MAX_HAM_BUTTON_NUMBER];   // this Array
    // reference cannot never be null as initialized
    ...
    public BoomMenuButton(Context context, AttributeSet attrs) {
        ...
        if (attr != null) {
            try {
                ...
                boomButtonPressedColor = attr.getColor(R.styleable
                                .BoomMenuButton_boom_button_pressed_color,
                        ContextCompat.getColor(mContext, R.color
                                .default_boom_button_color_pressed));
            } finally {
                attr.recycle();
            }
        }
        ...
        if (isInActionBar || isInList) {
            LayoutInflater.from(context).inflate(R.layout.boom_menu_button_in_action_bar, this,
                    true);
            ...
            if (android.os.Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
                ...
                LayoutInflater.from(context).inflate(R.layout.boom_menu_button_below_lollipop,
                        this, true);
            }
     ...
     * @param colors             The colors of sub buttons, including pressed-state and
     *                           normal-state.
     ...
     * @param colors     The colors(including the pressed-state and normal-state) of the sub
     *                   buttons.
        ...
        if (buttonType.equals(ButtonType.CIRCLE)) {
            ...
            if (showOrderType.equals(OrderType.DEFAULT)) {
                for (int i = 0; i < buttonNum; i++) {
                    ...
                    setShowAnimation(bars[i], hamButtons[i], originalLocations[i],
                            startLocations[i], endLocations[i], i);
                }
            } else if (showOrderType.equals(OrderType.REVERSE)) {
                for (int i = 0; i < buttonNum; i++) {
    ...
    private void startHideAnimations() {
        ...
        if (buttonType.equals(ButtonType.CIRCLE)) {
            if (hideOrderType.equals(OrderType.DEFAULT)) {
                for (int i = 0; i < buttonNum; i++) {
                    setHideAnimation(dots[i], circleButtons[i], endLocations[i],
                            startLocations[i], i);
                }
            ...
            if (hideOrderType.equals(OrderType.DEFAULT)) {
                ...
                for (int i = 0; i < buttonNum; i++) {
                    setHideAnimation(bars[i], hamButtons[i], endLocations[i], startLocations[i],
                            buttonNum - i - 1);
                }
            } else if (hideOrderType.equals(OrderType.RANDOM)) {
                Random random = new Random();
    ...
    public void setHideAnimation(
        ...
        if (button != null) {
            ObjectAnimator xAnimator = ObjectAnimator.ofFloat(button, "x", xs).setDuration
                    (duration);
            xAnimator.setStartDelay(index * delay);
            xAnimator.setInterpolator(InterpolatorFactory.getInterpolator(hideMoveEaseType));
            xAnimator.start();

            ObjectAnimator yAnimator = ObjectAnimator.ofFloat(button, "y", ys).setDuration
                    (duration);
    ...
    public ImageButton[] getImageButtons() {
        ...
        if (isInList || MEMORY_OPTIMIZATION) {
            //// ODO to return a weak ImageButton[] instead of an empty ImageButton[] if it can be
            // useful
        } else {
            for (int i = 0; i < buttonNum; i++) {
    ...
    public TextView[] getTextViews() {
        ...
        if (isInList || MEMORY_OPTIMIZATION) {
            // tTODO to return a weak TextView[] instead of an empty TextView[] if it can be useful
        } else {
            if (buttonType.equals(ButtonType.CIRCLE)) {
                for (int i = 0; i < buttonNum; i++)
     ...
     * @param colors
     */
    // tTODO for now doesnt work if MEMORY OPTIMIZATION is true
    ...
    public static class Builder {
        ...
        public Builder subButtons(ArrayList<Drawable> drawables, ArrayList<int[]> colors,
        ...
        public Builder boomButtonShadow(float boomButtonXOffsetOfShadow, float
                boomButtonYOffsetOfShadow) {
            this.boomButtonXOffsetOfShadow = boomButtonXOffsetOfShadow;
            this.boomButtonYOffsetOfShadow = boomButtonYOffsetOfShadow;
            return this;
        }

        public Builder subButtonsShadow(float subButtonsXOffsetOfShadow, float
                subButtonsYOffsetOfShadow) {
        ...
        public BoomMenuButton init(BoomMenuButton boomMenuButton) {
            ...
            boomMenuButton.setBoomButtonShadowOffset(boomButtonXOffsetOfShadow,
                    boomButtonYOffsetOfShadow);
            boomMenuButton.setSubButtonShadowOffset(subButtonsXOffsetOfShadow,
                    subButtonsYOffsetOfShadow);
